...
class Calculator:
    ...
    def test_valid_inputs_with_source_class(self, valid_test_class, valid_source_class):
        ...
        result = validate_signature_inputs(
            existing_test_class=valid_test_class, class_under_test=valid_source_class
        )
    ...
    def test_valid_inputs_with_completion_prompt(
        self, valid_test_class, valid_source_class
    ):
...
class TestExample:
    ...
    def helper_function(self):
        ...
        with pytest.raises(
            SignatureValidationError, match="must contain at least one test function"
        ):
    ...
    def test_invalid_source_class_syntax(self, valid_test_class):
        ...
        with pytest.raises(SignatureValidationError, match="Input validation failed"):
            validate_signature_inputs(
                existing_test_class=valid_test_class, class_under_test=invalid_source
            )

    def test_empty_completion_prompt(self, valid_test_class):
        """Test validation fails with empty completion prompt."""
        with pytest.raises(
            SignatureValidationError, match="Completion prompt cannot be empty"
        ):
            validate_signature_inputs(
                existing_test_class=valid_test_class, completion_prompt="   "
            )
...
class NotATest:
    pass
        ...
        with pytest.raises(
            SignatureValidationError, match="No valid test functions found"
        ):
...
def helper_function():
    return True
        ...
        with pytest.raises(
            SignatureValidationError, match="Could not extract test function name"
        ):
